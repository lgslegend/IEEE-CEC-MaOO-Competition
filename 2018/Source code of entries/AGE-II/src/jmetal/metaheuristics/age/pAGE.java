            ...
            if (useEpsilonBoxesArchive) {
                for(int i=0; i<archive.size(); i++) {
                    ...
                    if (true && 
                             ...
                             */
                            Math.random()< Math.pow( (evaluations/(double)maxEvaluations) , 10) &&  // 1: linear, 10: quite low first phase and then at 90% a chance of 35% to get kicked
        ...
        */
        ...
        if (!useEpsilonBoxesArchive) {
            ...
            if (true) // check and remove
                for(int i=0; i<archive.size(); i++) {
                    Solution s = archive.get(i);
                    if (debugPageOffline) System.out.println("offline: c="+i+" size="+archive.size()+" --> "+Arrays.toString(archive.get(i).getObjectives()));
                    if (!isSolutionInPreferred(s)) {
                        if (debugPageOffline) System.out.println("offline: -="+i+" size="+archive.size()+" --> "+Arrays.toString(archive.get(i).getObjectives()));
                        archive.remove(i);
                        if (debugPageOffline && i<archive.size()) System.out.println("offline: -    size="+archive.size()+" --> "+Arrays.toString(archive.get(i).getObjectives()));
                        i--;// needs to be decremented since the previous item at i was removed
                    }
//                    i++;
                }
...
//              pAGE_main.main(new String[]{"ZDT2", "originalParetoFronts\\ZDT2.pf", "30", "true", "true", "0.005", "true","10000","BinaryTournament2","foo"});   
              pAGE_main.main(new String[]{"DTLZ4_3D", "originalParetoFronts\\DTLZ2.3D.1000.pf", "30", "true", "true", "0.01", "true","50000","BinaryTournament2","foo"});   
